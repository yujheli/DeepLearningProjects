...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        schedule_fac = tf.placeholder(tf.float32,1)
        #teacher_force = tf.placeholder(tf.bool, shape=[])
        ...
        for i in range(0, self.n_caption_lstm_step): ## Phase 2 => only generate captions
            ...
            # with tf.device("/gpu:0"):
            if i==0:
                current_embed_2 = tf.nn.embedding_lookup(self.Wemb, caption[:, i])
        ...
        ############################# Implementation of Attention Based Model ######################################
                ...
                c = tf.nn.softmax(tf.tanh(tf.add(a,b))*self.V,1)*tf.reshape(attention_concate,[self.batch_size, self.n_video_lstm_step,self.dim_hidden])
        ...
        ############################# Implementation of Scheduling ######################################
                ...
                #L =tf.constant([True,False])
                # vv = tf.cond(tf.Variable(np.random.random()) > schedule_fac,tf.bool(True),tf.bool(False))

                output2, state2 = tf.cond( tf.random_uniform([1],minval=0.0,maxval=1.0)[0]<schedule_fac[0],lambda: self.lstm2(tf.concat([current_embed, output1],1), state2),lambda: self.lstm2(tf.concat([current_embed_2, output1],1), state2))

                # if np.random.random() >= schedule_fac:
                #     output2, state2 = self.lstm2(tf.concat([current_embed, output1],1), state2)
                # else:
                #     output2, state2 = self.lstm2(tf.concat([output2, output1],1), state2)
            ...
            current_embed_2 = tf.nn.embedding_lookup(self.Wemb, max_prob_index)
        ...
        return loss, video, video_mask, caption, caption_mask, probs, schedule_fac
    ...
    def build_generator(self):
        ...
        for i in range(0, self.n_caption_lstm_step):
            ...
            ############################# Implementation of Attention Based Model ######################################
                ...
                c = tf.nn.softmax(tf.tanh(tf.add(a,b))*self.V,1)*tf.reshape(attention_concate,[1, self.n_video_lstm_step,self.dim_hidden])
...
def train(mode):
    ...
    tf_loss, tf_video, tf_video_mask, tf_caption, tf_caption_mask, tf_probs, tf_sche = model.build_model()
    ...
    for epoch in range(0, n_epochs):
        ...
        loss_to_draw_epoch = []
        #train_label_cur = [i[0] for i in train_label]

        train_label_cur = [i[np.random.choice(len(i))] for i in train_label]
        # # train_label_cur = [i[np.random.choice(len(i))] for i in train_label]
        # if (epoch > 100) and (epoch%10==0) and teacher_v>=0.5:
        #         teacher_v*=0.99
        ...
        if np.mod(epoch, 100) == 0:
            ...
            saver.save(sess, os.path.join(model_path, 'model_last'), global_step=epoch)
...
def test_peer():
    ...
    test_name = np.array(pd.read_csv(sys.argv[2]+'/peer_review_id.txt',header=None)[0])
    test_data = []
    for i in test_name:
        test_data.append(np.load(sys.argv[2]+'/peer_review/feat/'+i+'.npy'))
    test_data = np.array(test_data)
